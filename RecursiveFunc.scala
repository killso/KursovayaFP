...
object RecursiveFunctions {
   ...
   */
  def reverse[A](list: List[A]): List[A] = {
    def reverse1(a: A, l: List[A]): List[A] = Cons(a, l)
    @tailrec
   ...
   */
  def map[A, B](list: List[A])(f: A => B): List[B] = {
    def reverse1(a: A, l: List[B]): List[b] = Cons(f(a), l)
    @tailrec
    def loop2(l: List[A], num: List[B]): List[B] = l match {
      case Nil() => reverse(num)
      case Cons(x,y) => loop2(y,reverse1(x,num))
    }
    loop2(list,Nil())
   ...
   *    она получает функцию,
   * которая создает новый List[B] для каждого элемента типа A в списке. Поэтому вы создаете List[List[B]].
   */
  def flatMap[A,B](list: List[A])(f: A=>List[B]): List[B] = {
    def reverse2(a: A, l: List[B]): List[B] = append(f(a),l]
    @tailrec
    def loop2(l: List[A],num: List[B]): List[B] = l match {
      case Nil() => reverse(num)
      case Cons(x,y) => loop2(y,reverse2(x,num))
    }
    loop2(list, Nil())
  }
